...
const options = {
  key: fs.readFileSync(path.join(__dirname, './crt/arc-dev.jalaera.com.com-key.pem')),
  cert: fs.readFileSync(path.join(__dirname, './crt/arc-dev.jalaera.com.com.pem')),
};
app.use(
  '/',
  createProxyMiddleware({
...
server.on('upgrade', (req, socket, head) => {
  ...
  proxy.on('proxyReqWs', function (proxyReq) {
    proxyReq.setHeader('Origin', 'https://arc-dev.jalaera.com.com');
  });
...
server.listen(9015, () => {
  console.log('Express server proxying Vite now running at: https://arc-dev.jalaera.com.com');
});
